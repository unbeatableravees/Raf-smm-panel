...
if(!defined('BASEPATH')) {
  ...
  if( $admin["access"]["users"] != 1  ):
    header("Location:".site_url("admin/access"));
    exit();
  endif;
...
$id = route(3);
if( $_POST ):
$discount_percent = $_POST["discount_percentage_value"];

$conn->beginTransaction();
$insert = $conn->prepare("UPDATE clients SET discount_percentage=:discount_percentage WHERE client_id=:id ");
$insert = $insert-> execute(array("discount_percentage"=>$discount_percent ,"id"=>$id));
...
$_SESSION["msmbilisim_userlogin"]      = 1;
  ...
  elseif( $action == "edit" ):
    $user_id  = route(3);
    if( !countRow(["table"=>"clients","where"=>["client_id"=>$user_id]]) ): header("Location:".site_url("admin/clients")); exit(); endif;
    ...
    $client_access  = json_decode($client_detail["access"],true);
        if( $_POST ):
          ...
          if( !email_check($email) ){
            ...
            $insert = $conn->prepare("UPDATE clients SET username=:username,name=:name, balance=:balance, balance_type=:balance_type, debit_limit=:debit_limit, email=:email, telephone=:phone, register_date=:date, tel_type=:tel_type, email_type=:email_type, access=:access WHERE client_id=:id ");
            $insert = $insert-> execute(array("id"=>route(3),"username"=>$username,"balance_type"=>$debit,"debit_limit"=>$debit_limit,"balance"=>$balance,"name"=>$name,"email"=>$email,"phone"=>$tel,"date"=>date("Y.m.d H:i:s"),'tel_type'=>$tel_type,'email_type'=>$email_type,'access'=>json_encode($access) ));
            if( $insert ):
              $conn->commit();
              $referrer = site_url("admin/clients");
              $error    = 1;
    ...
    $client_access  = json_decode($client_detail["access"],true);
        if( $_POST ):
          ...
          if( strlen($password) < 8 ){
            ...
            $insert = $insert-> execute(array("username"=>$username,"pass"=>md5($password)));
            // $conn->commit();
            if( $insert ):
              $conn->commit();
              $referrer = site_url("admin/clients");
